...
namespace SBSWebProject.AutoMappingObject.ModelToEntity
{
    public class UsersMapper : IMappingObject<Web.Api.Models.Users, Users>
    {
        ...
        public Users Map(Web.Api.Models.Users objectToMap, Users refObj)
        {
            if (objectToMap == null) return null;
            var cityMapping = new CityMapper();
            var sbsBranchTeamMapper = new SbsBranchTeamMapper();
            ...
            refObj.ActivityStatusId = objectToMap.ActivityStatusId;
            refObj.BirthDate = objectToMap.BirthDate;
            refObj.BirthPalaceCityItem = cityMapping.Map(objectToMap.BirthPalaceCityItem);
            refObj.LivingCityItem = cityMapping.Map(objectToMap.LivingCityItem);
            refObj.Email = objectToMap.Email;
            refObj.EnFamily = objectToMap.EnFamily;
            refObj.EnName = objectToMap.EnName;
            refObj.FaFamily = objectToMap.FaFamily;
            refObj.FaName = objectToMap.FaName;
            refObj.FatherName = objectToMap.FatherName;
            refObj.IdNumber = objectToMap.IdNumber;
            refObj.IsMain = objectToMap.IsMain;
            ...
            refObj.Sex = objectToMap.Sex;
            refObj.Tel = objectToMap.Tel;
            refObj.Username = objectToMap.Username;
            refObj.InfoStatusId = objectToMap.InfoStatusId;//TODO
            refObj.NationalityCountryId = objectToMap.NationalityCountryId;
            refObj.PassengerId = objectToMap.PassengerId;//TODO
            //SbsBranchTeamS = objectToMap.SbsBranchTeamS.Select(item => sbsBranchTeamMapper.Map(item)).ToList();
            //RequestAirplaneServiceS = objectToMap.RequestAirplaneServiceS.Select(item => requestAirplaneServiceMapper.Map(item)).ToList();
            //UserLoginLogS = objectToMap.UserLoginLogS.Select(item => userLoginLogMapper.Map(item)).ToList();
            //UserPassengerS = objectToMap.UserPassengerS.Select(item => userPassengerMapper.Map(item)).ToList();
