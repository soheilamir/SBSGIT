...
{
    public class SbsBranchesDomainEntity : IDomainEntity<SBSWebProject.Data.Entities.SbsBranches>
    {
        private Type persitentType = typeof(SbsBranches);

        private readonly ISession _session;
        public SbsBranchesDomainEntity(ISession session)
        {
            _session = session;
        }
        ...
        public void Delete(SbsBranches entity)
        {
            GetSession().Delete(entity);
        }

        public System.Collections.IList GetByExample(SbsBranches exampleInstance, string[] propertiesToExclude)
        {
        ...
        public System.Collections.IList GetByExampleExactValue(SbsBranches exampleInstance, string[] propertiesToExclude)
        {
            ISession session = GetSession();
            ICriteria criteria = session.CreateCriteria(persitentType);
            Example example = Example.Create(exampleInstance);
        ...
        public SbsBranches GetEntityByID(long id)
        {
            ISession session = this.GetSession();
            return (SbsBranches)session.Load(persitentType, id);
        }

        public System.Collections.IList GetEntityS()
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<SbsBranches>().List();
        ...
        public void Save(SbsBranches entity)
        {
        ...
        public void Update(SbsBranches entity)
        {
            ISession session = GetSession();
        ...
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<SbsBranches>().Add(Restrictions.Eq("State", state)).List();
        }

        public void DeleteByState(SbsBranches entity)
        {
            entity.State = 1;
            ISession session = GetSession();
            var tx = session.BeginTransaction();
