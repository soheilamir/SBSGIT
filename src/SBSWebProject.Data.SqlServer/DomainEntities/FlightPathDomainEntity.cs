...
{
    public class FlightPathDomainEntity : IDomainEntity<SBSWebProject.Data.Entities.FlightPath>
    {
        private Type persitentType = typeof(FlightPath);

        private readonly ISession _session;
        public FlightPathDomainEntity(ISession session)
        {
            _session = session;
        }
        ...
        public System.Collections.IList GetByExample(FlightPath exampleInstance, string[] propertiesToExclude)
        {
        ...
        public System.Collections.IList GetByExampleExactValue(FlightPath exampleInstance, string[] propertiesToExclude)
        {
            ISession session = GetSession();
            ICriteria criteria = session.CreateCriteria(persitentType);
            Example example = Example.Create(exampleInstance);
        ...
        public FlightPath GetEntityByID(long id)
        {
            ISession session = this.GetSession();
            return (FlightPath)session.Load(persitentType, id);
        }

        public System.Collections.IList GetEntityS()
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<FlightPath>().List();
        ...
        public void Save(FlightPath entity)
        {
        ...
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<FlightPath>().Add(Restrictions.Eq("State", state)).List();
        }

        public void DeleteByState(FlightPath entity)
        {
            entity.State = 1;
            ISession session = GetSession();
            var tx = session.BeginTransaction();
