...
{
    public class JobOpportunityDomainEntity : IDomainEntity<SBSWebProject.Data.Entities.JobOpportunity>
    {
        private Type persitentType = typeof(JobOpportunity);

        private readonly ISession _session;
        public JobOpportunityDomainEntity(ISession session)
        {
            _session = session;
        }
        ...
        public System.Collections.IList GetByExample(JobOpportunity exampleInstance, string[] propertiesToExclude)
        {
        ...
        public System.Collections.IList GetByExampleExactValue(JobOpportunity exampleInstance, string[] propertiesToExclude)
        {
            ISession session = GetSession();
            ICriteria criteria = session.CreateCriteria(persitentType);
            Example example = Example.Create(exampleInstance);
        ...
        public JobOpportunity GetEntityByID(long id)
        {
            ...
            return (JobOpportunity)session.Load(persitentType, id);
        }

        public System.Collections.IList GetEntityS()
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<JobOpportunity>().List();
        ...
        public void Save(JobOpportunity entity)
        {
        ...
        public void Update(JobOpportunity entity)
        {
            ISession session = GetSession();
        ...
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<JobOpportunity>().Add(Restrictions.Eq("State", state)).List();
        }

        public void DeleteByState(JobOpportunity entity)
        {
            entity.State = 1;
            ISession session = GetSession();
            var tx = session.BeginTransaction();
