...
{
    public class SbsSectionsDomainEntity : IDomainEntity<SBSWebProject.Data.Entities.SbsSections>
    {
        private Type persitentType = typeof(SbsSections);

        private readonly ISession _session;
        public SbsSectionsDomainEntity(ISession session)
        {
            _session = session;
        }
        ...
        public void Delete(SbsSections entity)
        {
            GetSession().Delete(entity);
        }

        public System.Collections.IList GetByExample(SbsSections exampleInstance, string[] propertiesToExclude)
        {
        ...
        public System.Collections.IList GetByExampleExactValue(SbsSections exampleInstance, string[] propertiesToExclude)
        {
            ISession session = GetSession();
            ICriteria criteria = session.CreateCriteria(persitentType);
            Example example = Example.Create(exampleInstance);
        ...
        public SbsSections GetEntityByID(long id)
        {
            ISession session = this.GetSession();
            return (SbsSections)session.Load(persitentType, id);
        }

        public System.Collections.IList GetEntityS()
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<SbsSections>().List();
        ...
        public void Save(SbsSections entity)
        {
        ...
        public void Update(SbsSections entity)
        {
            ISession session = GetSession();
        ...
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<SbsSections>().Add(Restrictions.Eq("State", state)).List();
        }

        public void DeleteByState(SbsSections entity)
        {
            entity.State = 1;
            ISession session = GetSession();
            var tx = session.BeginTransaction();
