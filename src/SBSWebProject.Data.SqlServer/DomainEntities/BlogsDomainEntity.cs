...
{
    public class BlogsDomainEntity : IDomainEntity<SBSWebProject.Data.Entities.Blogs>
    {
        private Type persitentType = typeof(Blogs);

        private readonly ISession _session;
        public BlogsDomainEntity(ISession session)
        {
            _session = session;
        }
        ...
        public System.Collections.IList GetByExample(Blogs exampleInstance, string[] propertiesToExclude)
        {
        ...
        public System.Collections.IList GetByExampleExactValue(Blogs exampleInstance, string[] propertiesToExclude)
        {
            ISession session = GetSession();
            ICriteria criteria = session.CreateCriteria(persitentType);
            Example example = Example.Create(exampleInstance);
        ...
        public Blogs GetEntityByID(long id)
        {
            ISession session = this.GetSession();
            return (Blogs)session.Load(persitentType, id);
        }

        public System.Collections.IList GetEntityS()
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Blogs>().List();
        ...
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Blogs>().Add(Restrictions.Eq("State", state)).List();
        }

        public void DeleteByState(Blogs entity)
        {
            entity.State = 1;
            ISession session = GetSession();
            var tx = session.BeginTransaction();
