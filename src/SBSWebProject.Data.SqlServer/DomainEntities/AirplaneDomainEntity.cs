...
{
    public class AirplaneDomainEntity : IDomainEntity<SBSWebProject.Data.Entities.Airplane>
    {
        private Type persitentType = typeof(Airplane);

        private readonly ISession _session;
        public AirplaneDomainEntity(ISession session)
        {
            _session = session;
        }
        ...
        public System.Collections.IList GetByExample(Airplane exampleInstance, string[] propertiesToExclude)
        {
        ...
        public System.Collections.IList GetByExampleExactValue(Airplane exampleInstance, string[] propertiesToExclude)
        {
            ISession session = GetSession();
            ICriteria criteria = session.CreateCriteria(persitentType);
            Example example = Example.Create(exampleInstance);
        ...
        public Airplane GetEntityByID(long id)
        {
            ISession session = this.GetSession();
            return (Airplane)session.Load(persitentType, id);
        }

        public System.Collections.IList GetEntityS()
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Airplane>().List();
        ...
        public void Save(Airplane entity)
        {
        ...
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Airplane>().Add(Restrictions.Eq("State", state)).List();
        }

        public void DeleteByState(Airplane entity)
        {
            entity.State = 1;
            ISession session = GetSession();
            var tx = session.BeginTransaction();
