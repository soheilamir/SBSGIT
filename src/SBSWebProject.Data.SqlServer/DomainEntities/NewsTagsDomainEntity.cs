...
{
    public class NewsTagsDomainEntity : IDomainEntity<SBSWebProject.Data.Entities.NewsTags>
    {
        private Type persitentType = typeof(NewsTags);

        private readonly ISession _session;
        public NewsTagsDomainEntity(ISession session)
        {
            _session = session;
        }
        ...
        public void Delete(NewsTags entity)
        {
            GetSession().Delete(entity);
        }

        public System.Collections.IList GetByExample(NewsTags exampleInstance, string[] propertiesToExclude)
        {
        ...
        public System.Collections.IList GetByExampleExactValue(NewsTags exampleInstance, string[] propertiesToExclude)
        {
            ISession session = GetSession();
            ICriteria criteria = session.CreateCriteria(persitentType);
            Example example = Example.Create(exampleInstance);
        ...
        public NewsTags GetEntityByID(long id)
        {
            ISession session = this.GetSession();
            return (NewsTags)session.Load(persitentType, id);
        }

        public System.Collections.IList GetEntityS()
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<NewsTags>().List();
        ...
        public void Save(NewsTags entity)
        {
        ...
        public void Update(NewsTags entity)
        {
            ISession session = GetSession();
        ...
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<NewsTags>().Add(Restrictions.Eq("State", state)).List();
        }

        public void DeleteByState(NewsTags entity)
        {
            entity.State = 1;
            ISession session = GetSession();
            var tx = session.BeginTransaction();
