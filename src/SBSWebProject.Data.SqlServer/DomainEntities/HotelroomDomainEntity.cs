...
{
    public class HotelRoomDomainEntity : IDomainEntity<SBSWebProject.Data.Entities.HotelRoom>
    {
        private Type persitentType = typeof(HotelRoom);

        private readonly ISession _session;
        public HotelRoomDomainEntity(ISession session)
        {
            _session = session;
        }
        ...
        public System.Collections.IList GetByExample(HotelRoom exampleInstance, string[] propertiesToExclude)
        {
        ...
        public System.Collections.IList GetByExampleExactValue(HotelRoom exampleInstance, string[] propertiesToExclude)
        {
            ISession session = GetSession();
            ICriteria criteria = session.CreateCriteria(persitentType);
            Example example = Example.Create(exampleInstance);
        ...
        public HotelRoom GetEntityByID(long id)
        {
            ISession session = this.GetSession();
            return (HotelRoom)session.Load(persitentType, id);
        }

        public System.Collections.IList GetEntityS()
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<HotelRoom>().List();
        ...
        public void Save(HotelRoom entity)
        {
        ...
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<HotelRoom>().Add(Restrictions.Eq("State", state)).List();
        }

        public void DeleteByState(HotelRoom entity)
        {
            entity.State = 1;
            ISession session = GetSession();
            var tx = session.BeginTransaction();
