...
{
    public class AirportsDomainEntity : IDomainEntity<SBSWebProject.Data.Entities.Airports>
    {
        private Type persitentType = typeof(Airports);

        private readonly ISession _session;
        public AirportsDomainEntity(ISession session)
        {
            _session = session;
        }
        ...
        public System.Collections.IList GetByExample(Airports exampleInstance, string[] propertiesToExclude)
        {
        ...
        public System.Collections.IList GetByExampleExactValue(Airports exampleInstance, string[] propertiesToExclude)
        {
            ISession session = GetSession();
            ICriteria criteria = session.CreateCriteria(persitentType);
            Example example = Example.Create(exampleInstance);
        ...
        public Airports GetEntityByID(long id)
        {
            ISession session = this.GetSession();
            return (Airports)session.Load(persitentType, id);
        }

        public System.Collections.IList GetEntityS()
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Airports>().List();
        ...
        public void Save(Airports entity)
        {
        ...
        public void Update(Airports entity)
        {
            ISession session = GetSession();
        ...
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Airports>().Add(Restrictions.Eq("State", state)).List();
        }

        public void DeleteByState(Airports entity)
        {
            entity.State = 1;
            ISession session = GetSession();
            var tx = session.BeginTransaction();
