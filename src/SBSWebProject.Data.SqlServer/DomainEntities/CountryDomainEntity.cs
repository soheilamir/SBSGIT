...
{
    public class CountryDomainEntity : IDomainEntity<SBSWebProject.Data.Entities.Country>
    {
        private Type persitentType = typeof(Country);

        private readonly ISession _session;
        public CountryDomainEntity(ISession session)
        {
            _session = session;
        }
        ...
        public System.Collections.IList GetByExample(Country exampleInstance, string[] propertiesToExclude)
        {
        ...
        public System.Collections.IList GetByExampleExactValue(Country exampleInstance, string[] propertiesToExclude)
        {
            ISession session = GetSession();
            ICriteria criteria = session.CreateCriteria(persitentType);
            Example example = Example.Create(exampleInstance);
        ...
        public Country GetEntityByID(long id)
        {
            ...
            return (Country)session.Load(persitentType, id);
        }

        public System.Collections.IList GetEntityS()
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Country>().List();
        ...
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Country>().Add(Restrictions.Eq("State", state)).List();
        }

        public void DeleteByState(Country entity)
        {
            entity.State = 1;
            ISession session = GetSession();
            var tx = session.BeginTransaction();
