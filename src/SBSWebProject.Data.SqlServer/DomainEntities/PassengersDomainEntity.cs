...
{
    public class PassengersDomainEntity : IDomainEntity<SBSWebProject.Data.Entities.Passengers>
    {
        private Type persitentType = typeof(Passengers);

        private readonly ISession _session;
        public PassengersDomainEntity(ISession session)
        {
            _session = session;
        }
        ...
        public void Delete(Passengers entity)
        {
            GetSession().Delete(entity);
        }

        public System.Collections.IList GetByExample(Passengers exampleInstance, string[] propertiesToExclude)
        {
        ...
        public System.Collections.IList GetByExampleExactValue(Passengers exampleInstance, string[] propertiesToExclude)
        {
            ISession session = GetSession();
            ICriteria criteria = session.CreateCriteria(persitentType);
            Example example = Example.Create(exampleInstance);
        ...
        public Passengers GetEntityByID(long id)
        {
            ISession session = this.GetSession();
            return (Passengers)session.Load(persitentType, id);
        }

        public System.Collections.IList GetEntityS()
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Passengers>().List();
        ...
        public void Save(Passengers entity)
        {
        ...
        public void Update(Passengers entity)
        {
            ISession session = GetSession();
        ...
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Passengers>().Add(Restrictions.Eq("State", state)).List();
        }

        public void DeleteByState(Passengers entity)
        {
            entity.State = 1;
            ISession session = GetSession();
            var tx = session.BeginTransaction();
