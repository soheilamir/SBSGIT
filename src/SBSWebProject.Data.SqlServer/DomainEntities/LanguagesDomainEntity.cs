...
{
    public class LanguagesDomainEntity : IDomainEntity<SBSWebProject.Data.Entities.Languages>
    {
        private Type persitentType = typeof(Languages);

        private readonly ISession _session;
        public LanguagesDomainEntity(ISession session)
        {
            _session = session;
        }
        ...
        public System.Collections.IList GetByExample(Languages exampleInstance, string[] propertiesToExclude)
        {
        ...
        public System.Collections.IList GetByExampleExactValue(Languages exampleInstance, string[] propertiesToExclude)
        {
            ISession session = GetSession();
            ICriteria criteria = session.CreateCriteria(persitentType);
            Example example = Example.Create(exampleInstance);
        ...
        public Languages GetEntityByID(long id)
        {
            ...
            return (Languages)session.Load(persitentType, id);
        }

        public System.Collections.IList GetEntityS()
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Languages>().List();
        ...
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Languages>().Add(Restrictions.Eq("State", state)).List();
        }

        public void DeleteByState(Languages entity)
        {
            entity.State = 1;
            ISession session = GetSession();
            var tx = session.BeginTransaction();
