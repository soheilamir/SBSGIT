...
{
    public class ExtensionsDomainEntity : IDomainEntity<SBSWebProject.Data.Entities.Extensions>
    {
        private Type persitentType = typeof(Extensions);

        private readonly ISession _session;
        public ExtensionsDomainEntity(ISession session)
        {
            _session = session;
        }
        ...
        public void Delete(Extensions entity)
        {
            GetSession().Delete(entity);
        }

        public System.Collections.IList GetByExample(Extensions exampleInstance, string[] propertiesToExclude)
        {
        ...
        public System.Collections.IList GetByExampleExactValue(Extensions exampleInstance, string[] propertiesToExclude)
        {
            ISession session = GetSession();
            ICriteria criteria = session.CreateCriteria(persitentType);
            Example example = Example.Create(exampleInstance);
        ...
        public Extensions GetEntityByID(long id)
        {
            ISession session = this.GetSession();
            return (Extensions)session.Load(persitentType, id);
        }

        public System.Collections.IList GetEntityS()
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Extensions>().List();
        ...
        public void Save(Extensions entity)
        {
        ...
        public void Update(Extensions entity)
        {
            ISession session = GetSession();
        ...
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Extensions>().Add(Restrictions.Eq("State", state)).List();
        }

        public void DeleteByState(Extensions entity)
        {
            entity.State = 1;
            ISession session = GetSession();
            var tx = session.BeginTransaction();
