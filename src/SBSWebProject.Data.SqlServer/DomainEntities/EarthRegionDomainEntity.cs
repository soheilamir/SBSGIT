...
{
    public class EarthRegionDomainEntity : IDomainEntity<SBSWebProject.Data.Entities.EarthRegion>
    {
        private Type persitentType = typeof(EarthRegion);

        private readonly ISession _session;
        public EarthRegionDomainEntity(ISession session)
        {
            _session = session;
        }
        ...
        public void Delete(EarthRegion entity)
        {
            GetSession().Delete(entity);
        }

        public System.Collections.IList GetByExample(EarthRegion exampleInstance, string[] propertiesToExclude)
        {
        ...
        public System.Collections.IList GetByExampleExactValue(EarthRegion exampleInstance, string[] propertiesToExclude)
        {
            ISession session = GetSession();
            ICriteria criteria = session.CreateCriteria(persitentType);
            Example example = Example.Create(exampleInstance);
        ...
        public EarthRegion GetEntityByID(long id)
        {
            ISession session = this.GetSession();
            return (EarthRegion)session.Load(persitentType, id);
        }

        public System.Collections.IList GetEntityS()
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<EarthRegion>().List();
        ...
        public void Save(EarthRegion entity)
        {
        ...
        public void Update(EarthRegion entity)
        {
            ISession session = GetSession();
        ...
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<EarthRegion>().Add(Restrictions.Eq("State", state)).List();
        }

        public void DeleteByState(EarthRegion entity)
        {
            entity.State = 1;
            ISession session = GetSession();
            var tx = session.BeginTransaction();
