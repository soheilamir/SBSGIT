...
{
    public class OrdersDomainEntity : IDomainEntity<SBSWebProject.Data.Entities.Orders>
    {
        private Type persitentType = typeof(Orders);

        private readonly ISession _session;
        public OrdersDomainEntity(ISession session)
        {
            _session = session;
        }
        ...
        public System.Collections.IList GetByExample(Orders exampleInstance, string[] propertiesToExclude)
        {
        ...
        public System.Collections.IList GetByExampleExactValue(Orders exampleInstance, string[] propertiesToExclude)
        {
            ISession session = GetSession();
            ICriteria criteria = session.CreateCriteria(persitentType);
            Example example = Example.Create(exampleInstance);
        ...
        public Orders GetEntityByID(long id)
        {
            ...
            return (Orders)session.Load(persitentType, id);
        }

        public System.Collections.IList GetEntityS()
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Orders>().List();
        ...
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Orders>().Add(Restrictions.Eq("State", state)).List();
        }

        public void DeleteByState(Orders entity)
        {
            entity.State = 1;
            ISession session = GetSession();
            var tx = session.BeginTransaction();
