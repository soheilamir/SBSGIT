...
{
    public class WebPagesDomainEntity : IDomainEntity<SBSWebProject.Data.Entities.WebPages>
    {
        private Type persitentType = typeof(WebPages);

        private readonly ISession _session;
        public WebPagesDomainEntity(ISession session)
        {
            _session = session;
        }
        ...
        public void Delete(WebPages entity)
        {
            GetSession().Delete(entity);
        }

        public System.Collections.IList GetByExample(WebPages exampleInstance, string[] propertiesToExclude)
        {
        ...
        public System.Collections.IList GetByExampleExactValue(WebPages exampleInstance, string[] propertiesToExclude)
        {
            ISession session = GetSession();
            ICriteria criteria = session.CreateCriteria(persitentType);
            Example example = Example.Create(exampleInstance);
        ...
        public WebPages GetEntityByID(long id)
        {
            ISession session = this.GetSession();
            return (WebPages)session.Load(persitentType, id);
        }

        public System.Collections.IList GetEntityS()
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<WebPages>().List();
        ...
        public void Save(WebPages entity)
        {
        ...
        public void Update(WebPages entity)
        {
            ISession session = GetSession();
        ...
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<WebPages>().Add(Restrictions.Eq("State", state)).List();
        }

        public void DeleteByState(WebPages entity)
        {
            entity.State = 1;
            ISession session = GetSession();
            var tx = session.BeginTransaction();
