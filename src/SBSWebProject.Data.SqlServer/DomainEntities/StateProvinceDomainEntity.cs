...
{
    public class StateProvinceDomainEntity : IDomainEntity<SBSWebProject.Data.Entities.StateProvince>
    {
        private Type persitentType = typeof(StateProvince);

        private readonly ISession _session;
        public StateProvinceDomainEntity(ISession session)
        {
            _session = session;
        }
        ...
        public System.Collections.IList GetByExample(StateProvince exampleInstance, string[] propertiesToExclude)
        {
        ...
        public System.Collections.IList GetByExampleExactValue(StateProvince exampleInstance, string[] propertiesToExclude)
        {
            ISession session = GetSession();
            ICriteria criteria = session.CreateCriteria(persitentType);
            Example example = Example.Create(exampleInstance);
        ...
        public StateProvince GetEntityByID(long id)
        {
            ISession session = this.GetSession();
            return (StateProvince)session.Load(persitentType, id);
        }

        public System.Collections.IList GetEntityS()
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<StateProvince>().List();
        ...
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<StateProvince>().Add(Restrictions.Eq("State", state)).List();
        }

        public void DeleteByState(StateProvince entity)
        {
            entity.State = 1;
            ISession session = GetSession();
            var tx = session.BeginTransaction();
