...
{
    public class MessagesDomainEntity : IDomainEntity<SBSWebProject.Data.Entities.Messages>
    {
        private Type persitentType = typeof(Messages);

        private readonly ISession _session;
        public MessagesDomainEntity(ISession session)
        {
            _session = session;
        }
        ...
        public System.Collections.IList GetByExample(Messages exampleInstance, string[] propertiesToExclude)
        {
        ...
        public System.Collections.IList GetByExampleExactValue(Messages exampleInstance, string[] propertiesToExclude)
        {
            ISession session = GetSession();
            ICriteria criteria = session.CreateCriteria(persitentType);
            Example example = Example.Create(exampleInstance);
        ...
        public Messages GetEntityByID(long id)
        {
            ISession session = this.GetSession();
            return (Messages)session.Load(persitentType, id);
        }

        public System.Collections.IList GetEntityS()
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Messages>().List();
        ...
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Messages>().Add(Restrictions.Eq("State", state)).List();
        }

        public void DeleteByState(Messages entity)
        {
            entity.State = 1;
            ISession session = GetSession();
            var tx = session.BeginTransaction();
