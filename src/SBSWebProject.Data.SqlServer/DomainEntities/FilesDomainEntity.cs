...
{
    public class FilesDomainEntity : IDomainEntity<SBSWebProject.Data.Entities.Files>
    {
        private Type persitentType = typeof(Files);

        private readonly ISession _session;
        public FilesDomainEntity(ISession session)
        {
            _session = session;
        }
        ...
        public System.Collections.IList GetByExample(Files exampleInstance, string[] propertiesToExclude)
        {
        ...
        public System.Collections.IList GetByExampleExactValue(Files exampleInstance, string[] propertiesToExclude)
        {
            ISession session = GetSession();
            ICriteria criteria = session.CreateCriteria(persitentType);
            Example example = Example.Create(exampleInstance);
        ...
        public Files GetEntityByID(long id)
        {
            ISession session = this.GetSession();
            return (Files)session.Load(persitentType, id);
        }

        public System.Collections.IList GetEntityS()
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Files>().List();
        ...
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Files>().Add(Restrictions.Eq("State", state)).List();
        }

        public void DeleteByState(Files entity)
        {
            entity.State = 1;
            ISession session = GetSession();
            var tx = session.BeginTransaction();
