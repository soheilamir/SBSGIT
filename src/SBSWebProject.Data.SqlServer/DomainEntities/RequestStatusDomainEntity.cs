...
{
    public class RequestStatusDomainEntity : IDomainEntity<SBSWebProject.Data.Entities.RequestStatus>
    {
        private Type persitentType = typeof(RequestStatus);

        private readonly ISession _session;
        public RequestStatusDomainEntity(ISession session)
        {
            _session = session;
        }
        ...
        public System.Collections.IList GetByExample(RequestStatus exampleInstance, string[] propertiesToExclude)
        {
        ...
        public System.Collections.IList GetByExampleExactValue(RequestStatus exampleInstance, string[] propertiesToExclude)
        {
            ISession session = GetSession();
            ICriteria criteria = session.CreateCriteria(persitentType);
            Example example = Example.Create(exampleInstance);
        ...
        public RequestStatus GetEntityByID(long id)
        {
            ISession session = this.GetSession();
            return (RequestStatus)session.Load(persitentType, id);
        }

        public System.Collections.IList GetEntityS()
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<RequestStatus>().List();
        ...
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<RequestStatus>().Add(Restrictions.Eq("State", state)).List();
        }

        public void DeleteByState(RequestStatus entity)
        {
            entity.State = 1;
            ISession session = GetSession();
            var tx = session.BeginTransaction();
