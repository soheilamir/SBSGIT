...
{
    public class CompanyAttachmentDomainEntity : IDomainEntity<SBSWebProject.Data.Entities.CompanyAttachment>
    {
        private Type persitentType = typeof(CompanyAttachment);

        private readonly ISession _session;
        public CompanyAttachmentDomainEntity(ISession session)
        {
            _session = session;
        }
        ...
        public System.Collections.IList GetByExample(CompanyAttachment exampleInstance, string[] propertiesToExclude)
        {
        ...
        public System.Collections.IList GetByExampleExactValue(CompanyAttachment exampleInstance, string[] propertiesToExclude)
        {
            ISession session = GetSession();
            ICriteria criteria = session.CreateCriteria(persitentType);
            Example example = Example.Create(exampleInstance);
        ...
        public CompanyAttachment GetEntityByID(long id)
        {
            ISession session = this.GetSession();
            return (CompanyAttachment)session.Load(persitentType, id);
        }

        public System.Collections.IList GetEntityS()
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<CompanyAttachment>().List();
        ...
        public void Save(CompanyAttachment entity)
        {
        ...
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<CompanyAttachment>().Add(Restrictions.Eq("State", state)).List();
        }

        public void DeleteByState(CompanyAttachment entity)
        {
            entity.State = 1;
            ISession session = GetSession();
            var tx = session.BeginTransaction();
