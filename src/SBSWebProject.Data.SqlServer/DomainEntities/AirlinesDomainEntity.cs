...
{
    public class AirlinesDomainEntity : IDomainEntity<SBSWebProject.Data.Entities.Airlines>
    {
        private Type persitentType = typeof(Airlines);

        private readonly ISession _session;
        public AirlinesDomainEntity(ISession session)
        {
            _session = session;
        }
        ...
        public void Delete(Airlines entity)
        {
            GetSession().Delete(entity);
        }

        public System.Collections.IList GetByExample(Airlines exampleInstance, string[] propertiesToExclude)
        {
        ...
        public System.Collections.IList GetByExampleExactValue(Airlines exampleInstance, string[] propertiesToExclude)
        {
            ISession session = GetSession();
            ICriteria criteria = session.CreateCriteria(persitentType);
            Example example = Example.Create(exampleInstance);
        ...
        public Airlines GetEntityByID(long id)
        {
            ISession session = this.GetSession();
            return (Airlines)session.Load(persitentType, id);
        }

        public System.Collections.IList GetEntityS()
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Airlines>().List();
        ...
        public void Save(Airlines entity)
        {
        ...
        public void Update(Airlines entity)
        {
            ISession session = GetSession();
        ...
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Airlines>().Add(Restrictions.Eq("State", state)).List();
        }

        public void DeleteByState(Airlines entity)
        {
            entity.State = 1;
            ISession session = GetSession();
            var tx = session.BeginTransaction();
