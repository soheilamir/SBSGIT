...
{
    public class HotelsDomainEntity : IDomainEntity<SBSWebProject.Data.Entities.Hotels>
    {
        private Type persitentType = typeof(Hotels);

        private readonly ISession _session;
        public HotelsDomainEntity(ISession session)
        {
            _session = session;
        }
        ...
        public System.Collections.IList GetByExample(Hotels exampleInstance, string[] propertiesToExclude)
        {
        ...
        public System.Collections.IList GetByExampleExactValue(Hotels exampleInstance, string[] propertiesToExclude)
        {
            ISession session = GetSession();
            ICriteria criteria = session.CreateCriteria(persitentType);
            Example example = Example.Create(exampleInstance);
        ...
        public Hotels GetEntityByID(long id)
        {
            ISession session = this.GetSession();
            return (Hotels)session.Load(persitentType, id);
        }

        public System.Collections.IList GetEntityS()
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Hotels>().List();
        ...
        public void Save(Hotels entity)
        {
        ...
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Hotels>().Add(Restrictions.Eq("State", state)).List();
        }

        public void DeleteByState(Hotels entity)
        {
            entity.State = 1;
            ISession session = GetSession();
            var tx = session.BeginTransaction();
