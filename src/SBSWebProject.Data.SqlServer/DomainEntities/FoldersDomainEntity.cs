...
{
    public class FoldersDomainEntity : IDomainEntity<SBSWebProject.Data.Entities.Folders>
    {
        private Type persitentType = typeof(Folders);

        private readonly ISession _session;
        public FoldersDomainEntity(ISession session)
        {
            _session = session;
        }
        ...
        public void Delete(Folders entity)
        {
            GetSession().Delete(entity);
        }

        public System.Collections.IList GetByExample(Folders exampleInstance, string[] propertiesToExclude)
        {
        ...
        public System.Collections.IList GetByExampleExactValue(Folders exampleInstance, string[] propertiesToExclude)
        {
            ISession session = GetSession();
            ICriteria criteria = session.CreateCriteria(persitentType);
            Example example = Example.Create(exampleInstance);
        ...
        public Folders GetEntityByID(long id)
        {
            ISession session = this.GetSession();
            return (Folders)session.Load(persitentType, id);
        }

        public System.Collections.IList GetEntityS()
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Folders>().List();
        ...
        public void Save(Folders entity)
        {
        ...
        public void Update(Folders entity)
        {
            ISession session = GetSession();
        ...
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Folders>().Add(Restrictions.Eq("State", state)).List();
        }

        public void DeleteByState(Folders entity)
        {
            entity.State = 1;
            ISession session = GetSession();
            var tx = session.BeginTransaction();
