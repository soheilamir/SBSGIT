...
{
    public class SbsRolesDomainEntity : IDomainEntity<SBSWebProject.Data.Entities.SbsRoles>
    {
        private Type persitentType = typeof(SbsRoles);

        private readonly ISession _session;
        public SbsRolesDomainEntity(ISession session)
        {
            _session = session;
        }
        ...
        public void Delete(SbsRoles entity)
        {
            GetSession().Delete(entity);
        }

        public System.Collections.IList GetByExample(SbsRoles exampleInstance, string[] propertiesToExclude)
        {
        ...
        public System.Collections.IList GetByExampleExactValue(SbsRoles exampleInstance, string[] propertiesToExclude)
        {
            ISession session = GetSession();
            ICriteria criteria = session.CreateCriteria(persitentType);
            Example example = Example.Create(exampleInstance);
        ...
        public SbsRoles GetEntityByID(long id)
        {
            ISession session = this.GetSession();
            return (SbsRoles)session.Load(persitentType, id);
        }

        public System.Collections.IList GetEntityS()
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<SbsRoles>().List();
        ...
        public void Save(SbsRoles entity)
        {
        ...
        public void Update(SbsRoles entity)
        {
            ISession session = GetSession();
        ...
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<SbsRoles>().Add(Restrictions.Eq("State", state)).List();
        }

        public void DeleteByState(SbsRoles entity)
        {
            entity.State = 1;
            ISession session = GetSession();
            var tx = session.BeginTransaction();
