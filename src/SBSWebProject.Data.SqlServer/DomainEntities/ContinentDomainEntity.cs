...
{
    public class ContinentDomainEntity : IDomainEntity<SBSWebProject.Data.Entities.Continent>
    {
        private Type persitentType = typeof(Continent);

        private readonly ISession _session;
        public ContinentDomainEntity(ISession session)
        {
            _session = session;
        }
        ...
        public System.Collections.IList GetByExample(Continent exampleInstance, string[] propertiesToExclude)
        {
        ...
        public System.Collections.IList GetByExampleExactValue(Continent exampleInstance, string[] propertiesToExclude)
        {
            ISession session = GetSession();
            ICriteria criteria = session.CreateCriteria(persitentType);
            Example example = Example.Create(exampleInstance);
        ...
        public Continent GetEntityByID(long id)
        {
            ...
            return (Continent)session.Load(persitentType, id);
        }

        public System.Collections.IList GetEntityS()
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Continent>().List();
        ...
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Continent>().Add(Restrictions.Eq("State", state)).List();
        }

        public void DeleteByState(Continent entity)
        {
            entity.State = 1;
            ISession session = GetSession();
            var tx = session.BeginTransaction();
