...
{
    public class UsersDomainEntity : IDomainEntity<SBSWebProject.Data.Entities.Users>
    {
        private Type persitentType = typeof(Users);

        private readonly ISession _session;
        public UsersDomainEntity(ISession session)
        {
            _session = session;
        }
        ...
        public System.Collections.IList GetByExample(Users exampleInstance, string[] propertiesToExclude)
        {
        ...
        public System.Collections.IList GetByExampleExactValue(Users exampleInstance, string[] propertiesToExclude)
        {
            ISession session = GetSession();
            ICriteria criteria = session.CreateCriteria(persitentType);
            Example example = Example.Create(exampleInstance);
        ...
        public Users GetEntityByID(long id)
        {
            ...
            return (Users)session.Load(persitentType, id);
        }

        public System.Collections.IList GetEntityS()
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Users>().List();
        ...
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Users>().Add(Restrictions.Eq("State", state)).List();
        }

        public void DeleteByState(Users entity)
        {
            entity.State = 1;
            ISession session = GetSession();
            var tx = session.BeginTransaction();
