...
{
    public class ServicesDomainEntity : IDomainEntity<SBSWebProject.Data.Entities.Services>
    {
        private Type persitentType = typeof(Services);

        private readonly ISession _session;
        public ServicesDomainEntity(ISession session)
        {
            _session = session;
        }
        ...
        public System.Collections.IList GetByExample(Services exampleInstance, string[] propertiesToExclude)
        {
        ...
        public System.Collections.IList GetByExampleExactValue(Services exampleInstance, string[] propertiesToExclude)
        {
            ISession session = GetSession();
            ICriteria criteria = session.CreateCriteria(persitentType);
            Example example = Example.Create(exampleInstance);
        ...
        public Services GetEntityByID(long id)
        {
            ISession session = this.GetSession();
            return (Services)session.Load(persitentType, id);
        }

        public System.Collections.IList GetEntityS()
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Services>().List();
        ...
        {
            ISession session = this.GetSession();
            return session.CreateCriteria<Services>().Add(Restrictions.Eq("State", state)).List();
        }

        public void DeleteByState(Services entity)
        {
            entity.State = 1;
            ISession session = GetSession();
            var tx = session.BeginTransaction();
